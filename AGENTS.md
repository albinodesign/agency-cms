...
Im Editor gruppiert `detectPageLabel` (`src/components/editor/EditorClient.tsx`) die Sektionen per `page`-Angabe im Manifest (ohne Angabe greift die Schlüsselwort-Heuristik aus ID/Titel, sonst Dateiname) zu Seiten-Tabs (z. B. Startseite, Leistungen, Kontakt, Firmendaten); innerhalb einer Seite bleiben die Sektionen Akkordeons, die Suche durchsucht seitenübergreifend alle Felder. Alle Listen sind fest (kein Wachstum/Kürzen per Entwurf; neue Einträge legt die Agentur im Website-Repo an). Nach dem Veröffentlichen zeigt der Editor den echten Aufbau-Status (Poll alle 10 s, max. 18× = 3 Min. via `GET /api/site/[siteId]/deploy-status?sha=`) und lädt danach die Vorschau automatisch neu. Stille Manifest-Deutungen (unbekannte Typen → Text, weggelassene Einträge, Fallback-Kennungen) zeigt der Editor als bernsteinfarbene Warnbox mit Anzahl und Stelle (`normalizeManifestWithWarnings` in `src/lib/github.ts`). Vorschau-Navigation gegen die document.referrer-Falle (CMS-REFERENCE.md Abschnitt 7): Der Seiten-Tab-Wechsel lädt die Vorschau immer CMS-seitig (`iframe.src` direkt auf die Ziel-URL aus `getPagePreviewUrl` in `src/lib/preview-url.ts`: `src/content/pages/<slug>.json` → `<preview_url>/<slug>`, Startseiten-Slugs → Basis-URL), damit `document.referrer` die CMS-Domain bleibt und Finden-Klicks als `CMS_FIELD_SELECT` ankommen; Navigation über In-Preview-Links erkennt der `onLoad`-Handler und meldet ehrlich per Toast (mit „Neu laden“-Knopf in der Vorschau-Leiste zur CMS-seitigen Wiederherstellung). Brücken ab Referenz v1.2 melden `CMS_BRIDGE_READY` (gemerkter CMS-Origin statt Referrer) – dann entfallen Warn-Hinweis, weil Finden-Klicks überall ankommen.
