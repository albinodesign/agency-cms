...
3. `POST /api/publish` gruppiert die Entwürfe nach Zieldatei, schreibt die Werte per Dot-Path (`src/lib/json-path.ts`) in die JSON-Dateien, committet sie mit `cms: update content by client` auf `main`, speichert einen Snapshot (vollständige Datei-Inhalte, verschachtelt nach Dateipfad) in `publish_history` und löscht anschließend nur exakt die publizierten Entwürfe anhand ihrer Primärschlüssel (`in("id", committedDraftIds)`), damit während des Publish-Vorgangs neu getippte Entwürfe nicht verloren gehen. Felder ohne Manifest-Zuordnung werden übersprungen, bleiben als Entwurf erhalten und werden in der Antwort vermerkt.
...
Der Editor rendert seine Felder aus `src/content/cms.manifest.json` im jeweiligen Website-Repo. Felder haben `id`, `label`, `type` (`text` | `textarea` | `image`), `file` (Zieldatei im Repo) und `path` (Dot-Path im JSON), optional `placeholder` und `maxLength`. `features.blog: true` (oder `{ blog: { enabled: true } }`) aktiviert den Blog-Tab. `normalizeManifest` in `src/lib/github.ts` akzeptiert toleranterweise mehrere Formate (Array von Sektionen/Feldern, `{ sections }`, `{ fields }`) und filtert ungültige Einträge heraus; Sektionen ohne gültige Felder fallen weg. Der Sektionsname wird aus `title` oder `label` (Fallback: `id`, sonst `Sektion N`) übernommen.
