...
3. `POST /api/publish` lädt das Manifest einmalig vom Server (`getManifestRaw`), prüft die Roh-Definition streng (`src/lib/content-guard.ts`: erlaubte Zieldateien `src/content/site.json` + `src/content/pages/*.json`, sichere Pfade, eindeutige IDs/Ziele, unterstützte Typen, tatsächliche Pfad-Existenz), gruppiert die Entwürfe nach Zieldatei, verarbeitet ausschließlich `.json`-Dateien per `JSON.parse` (andere Dateien werden übersprungen), schreibt die Werte per sicherem Pfad (`src/lib/json-path.ts`: `__proto__`/`constructor`/`prototype` verboten, nur eigene Eigenschaften, Listen nur im Bestand oder exakt am Ende) in die JSON-Dateien, committet sie mit `cms: update content by client` auf `main`, speichert einen Snapshot (vollständige Datei-Inhalte, verschachtelt nach Dateipfad) in `publish_history` und löscht anschließend nur exakt die publizierten Entwürfe anhand ihrer Primärschlüssel (`in("id", committedDraftIds)`), damit während des Publish-Vorgangs neu getippte Entwürfe nicht verloren gehen. Jeder Ziel-/Wertfehler bricht den gesamten Satz mit 400 ab (Fail-Closed, keine Teilspeicherung, Entwürfe bleiben erhalten); unbekannte Entwurfs-IDs ohne Manifest-Zuordnung bleiben als Entwurf erhalten und werden in der Antwort offengelegt.
